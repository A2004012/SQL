...
select * from flights
where departuretime >'2023-01-30 00:00:00' and departurecity = 'new york';
...
select  flightnumber , sum(ticketprice)
from flights
...
SELECT ARRIVALCITY,MAX(TICKETPRICE)
FROM FLIGHTS
...
select departurecity, avg(availableseats),sum(availableseats * ticketprice)
from flights
group by departurecity
...
select aircrafttype,count(flightnumber) as flightnumber
from flights
group by aircrafttype
order by flightnumber desc limit 5;
-- 7.	Retrieve flights operated by 'Emirates' airline.
select * from flights
where airlinename = 'emirates';
-- 8.	Find the maximum number of available seats among all flights.
select flightnumber,max(availableseats) 
from flights
group by flightnumber;
-- 9.	Display flights arriving in 'Los Angeles' with arrival time before '2023-05-01 00:00:00'.
select * from flights
...
select arrivalcity,count(flightid) 
from flights
group by arrivalcity
...
select departurecity,sum(ticketprice) as ticketprice
from flights
group by departurecity
...
select airlinename,avg(TicketPrice),count(flightnumber) 
from flights
group by airlinename
having count(flightnumber) >5;

select * from flights;
-- 14.	Display the arrival cities with the lowest average ticket prices.
select arrivalcity, avg(ticketprice) as ticketprice
from flights
group by arrivalcity
...
-- 15.	Find departure cities and the total number of flights departing from each city, limiting the result to the top 3 cities.
   select departurecity,count(flightnumber) as departurecity
   from flights
   group by departurecity
   order by departurecity desc limit 3;
...
select AirlineName, count(flightid)
from flights
group by AirlineName
...
select DepartureCity,avg(TicketPrice) as ticketprice
from flights
group by DepartureCity
order by TicketPrice asc;
...
select DepartureCity,sum(TicketPrice * availableseats)
from flights
group by DepartureCity
...
select AirlineName,sum(TicketPrice*AvailableSeats) as airlinename
from flights
group by AirlineName
having AirlineName > 8;

select airlinename,sum(ticketprice),count(AirlineName)
from flights
group by airlinename
having count(AirlineName)>8;



-- 23.	Display arrival cities and the latest arrival time among flights for each city.
select ArrivalCity,max(ArrivalTime)
from flights
group by arrivalcity
...
select DepartureCity,sum(TicketPrice * AvailableSeats) as orderby_revenue
from flights
group by DepartureCity
...
select departurecity,count(FlightNumber) as flightNumber 
from flights
group by DepartureCity 
...
select ArrivalCity,avg(TicketPrice)
from flights
group by arrivalcity
...
select AirlineName,sum(TicketPrice)as ticketprice
from flights
group by AirlineName 
order by ticketprice asc;
...
select  departurecity,sum(TicketPrice ) 
from flights
group by DepartureCity
...
select AircraftType,avg(TicketPrice)
from flights
group by AircraftType
...
select ArrivalCity,count(FlightNumber) as arrivalcity
from flights
group by ArrivalCity
order by arrivalcity desc limit 3;
...
select AirlineName,sum(TicketPrice),avg(TicketPrice)
from flights
group by AirlineName
...
select DepartureCity,min(departuretime),count(DepartureCity)
from flights
group by Departurecity
...
select  AirlineName,avg(TicketPrice) as ticketprice
from flights
group by AirlineName
order by ticketprice desc;
...
select  DepartureCity,sum(AvailableSeats)
from flights
group by DepartureCity
...
select Arrivalcity,max(arrivaltime) as arrivaltime 
from flights
group by Arrivalcity
order by max(ArrivalTime) desc;
-- 37.	Retrieve airlines and the maximum ticket price among their flights, limiting the result to the top 4 airlines.
select AirlineName,max(TicketPrice)
from flights
group by AirlineName limit 4;
...
select DepartureCity,avg(AvailableSeats)
from flights
group by DepartureCity
...
select ArrivalCity,sum(TicketPrice)
from flights
group by ArrivalCity
having sum(TicketPrice)>90000;
-- 40.	Calculate the average ticket price for each aircraft type, but include only types with an average ticket price greater than $1100 and order
 -- by average ticket price in descending order
 select aircrafttype,avg(ticketprice)
 from flights
 group by aircrafttype
 having avg(ticketprice)>1100
